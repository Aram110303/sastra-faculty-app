...
Original file is located at
    https://colab.research.google.com/drive/1RBdJd-CziXfHiPHeH3bdKUCteX8l_GQi
"""
...
# ----------------------------------
# SCRAPER FUNCTION FOR SASTRA SCBT
# ----------------------------------
...
def scrape_sastra_faculty(url, user_email):
    try:
        ...
        for prof in faculty_blocks:
            ...
            # Try to match email username to name
            ...
            if username in name.lower().replace(" ", ""):
                ...
                if "Areas of Interest" in text:
                    ...
                    areas = areas.split("ORCID")[0].strip()
                ...
                # Generate research summary from areas
                research_summary = f"Research focused on {areas} with interdisciplinary applications."
...
if st.session_state.page == "login":
    ...
    if st.button("Continue"):
        if not email.endswith("@sastra.edu"):
...
# ----------------------------------
    ...
    faculty_url = st.text_input("Faculty Profile URL")
    ...
    if st.button("Submit"):
        if not scopus_id:
            ...
            faculty_url=?
            WHERE email=?
            """, (scopus_id, orcid_id, scholar_link,
                  researchgate_link, faculty_url,
                  st.session_state.email))
            conn.commit()
...
# ----------------------------------
    ...
    if "error" not in profile:
        ...
        st.write(profile["areas_of_interest"])

        st.subheader("ORCID:")
        st.write(profile["orcid_id"])
